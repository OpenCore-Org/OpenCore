...
int main(int argc, char** argv, char** env) {
    ...
    Instruction_Type my_itype = VECTOR;     // change as needed
    Decoded_Instruction my_instr;

    // Fill out my_instr.vector_instr (or whatever type you have)

    uint32_t encoded_instruction_first_32bits;
    uint32_t encoded_instruction_second_32bits;
    bool is_64_instruction;

    // if(encode_instructions(my_itype, my_instr, encoded_instruction_first_32bits, encoded_instruction_second_32bits, is_64_instruction)){
    //     std::cerr << "Error encoding Instruction " << std::endl;
    // }

    // std::cout << "Instruction: " << std::hex << encoded_instruction_first_32bits;
    // if(is_64_instruction){
    //     std::cout << " " << encoded_instruction_second_32bits;
    // }
    // std::cout << std::endl;

    encoded_instruction_first_32bits = 0xDC308000;
    encoded_instruction_second_32bits = 0x037D0003;
    is_64_instruction = true;
    decode_instructions(encoded_instruction_first_32bits, encoded_instruction_second_32bits, is_64_instruction, my_itype, my_instr);

    std::cout << convert_enum_string(my_itype) << std::endl;
    print_decoded_instructions(my_instr.flat_instr);
