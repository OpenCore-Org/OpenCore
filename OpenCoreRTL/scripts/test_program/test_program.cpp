...
void print_instruction(Instruction_Type instruction_type, Decoded_Instruction &decoded_instr, std::ofstream &outfile1, std::ofstream &outfile2){
    ...
    outfile2 << std::hex << std::setw(8) << std::setfill('0') << encoded_instruction_first_32bits << std::endl;
    if(is_64_instruction){
        ...
        outfile2 << std::hex << std::setw(8) << std::setfill('0') << encoded_instruction_second_32bits << std::endl;
    }
}
...
#define RANDOM_NUM_A 10
#define RANDOM_NUM_B 20
#define RANDOM_FLOAT_A 5.5
#define RANDOM_FLOAT_B 10.3
...
int main(int argc, char** argv, char** env) {
    ...
    std::ofstream out_file("test_program.bin");
    std::ofstream out_file2("test_program.txt");

    // s2 = RANDOM_NUM_A
    my_itype = SCALAR;
    make_sop1_instr(my_instr, 3, 2, 255);   //opcode 3 (S_MOV_B32), dest 2, scr0 255 (literal)
    my_instr.scalar_instr.literal = RANDOM_NUM_A;
    my_instr.scalar_instr.literal_in_use = 1;
    print_instruction(my_itype, my_instr, out_file, out_file2);

    // s3 = RANDOM_NUM_B
    my_itype = SCALAR;
    make_sop1_instr(my_instr, 3, 3, 255);   //opcode 3 (S_MOV_B32), dest 3, scr0 255 (literal)
    my_instr.scalar_instr.literal = RANDOM_NUM_B;
    my_instr.scalar_instr.literal_in_use = 1;
    print_instruction(my_itype, my_instr, out_file, out_file2);

    // s4 = s2 + s3
    my_itype = SCALAR;
    make_sop2_instr(my_instr, 0, 4, 2, 3);   //opcode 0 (S_ADD_U32), dest s4, src0 s2, scr1 s3
    print_instruction(my_itype, my_instr, out_file, out_file2);

    // s5 = s3 - s2
    my_itype = SCALAR;
    make_sop2_instr(my_instr, 1, 5, 3, 2);   //opcode 1 (S_SUB_U32), dest s5, src0 s3, scr1 s2
    print_instruction(my_itype, my_instr, out_file, out_file2);

    // s6 = s2 * s3
    my_itype = SCALAR;
    make_sop2_instr(my_instr, 38, 6, 2, 3);   //opcode 38 (S_MUL_I32), dest s6, src0 s2, scr1 s3
    print_instruction(my_itype, my_instr, out_file, out_file2);
    ...
    // should write 0..31 to MEM[RANDOM_NUM_B + 0..31]
    my_itype2 = FLAT; 
    make_global_load_dword_instr(my_instr2, 21, 3, 1);   // v21 = MEM[s3 + v1]
    my_instr2.flat_instr.opcode = 28;   // GLOBAL_STORE_DWORD
    my_instr2.flat_instr.data = 1;      // store from v1
    print_instruction(my_itype2, my_instr2, out_file, out_file2);

    // should read the values stored by previous write into v2
    my_itype2 = FLAT; 
    make_global_load_dword_instr(my_instr2, 2, 3, 1);   // v2 = MEM[s3 + v1]
    print_instruction(my_itype2, my_instr2, out_file, out_file2);

    // v3 = v1 + v2
    my_itype = VECTOR;
    make_vop2_instr(my_instr, 37, 3, 256+1, 2);   //opcode 37 (v_add_nc_u32), vdest v3, src0 v1, vsrc1 v2
    print_instruction(my_itype, my_instr, out_file, out_file2);

    // load RANDOM_FLOAT_A into v5
    my_itype2 = VECTOR;
    make_vop1_instr(my_instr2, 1, 5, 255);  // opcode 1 (V_MOV_B32), dest v5, src0 255 (literal)
    my_instr2.vector_instr.literal_in_use = 1;
    my_instr2.vector_instr.literal = RANDOM_FLOAT_A;
    print_instruction(my_itype2, my_instr2, out_file, out_file2);

    // load RANDOM_FLOAT_B into v6
    my_itype2 = VECTOR;
    make_vop1_instr(my_instr2, 1, 6, 255);  // opcode 1 (V_MOV_B32), dest v5, src0 255 (literal)
    my_instr2.vector_instr.literal_in_use = 1;
    my_instr2.vector_instr.literal = RANDOM_FLOAT_B;
    print_instruction(my_itype2, my_instr2, out_file, out_file2);

    // v7 = v6*v5 
    my_itype = VECTOR;
    make_vop2_instr(my_instr, 8, 7, 256+5, 6);     //opcode 8 (V_MUL_F32), vdest v7, src0 v5, vsrc1 v6
    print_instruction(my_itype, my_instr, out_file, out_file2);
